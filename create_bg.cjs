...
code = code.replace("dots.forEach((d, i) => d.classList.toggle('active', i === idx));", "if (dots && dots.length) dots.forEach((d, i) => d.classList.toggle('active', i === idx));");
code = code.replace("dots.forEach(dot => {", "if (dots && dots.length) dots.forEach(dot => {");
