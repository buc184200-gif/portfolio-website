...
if (container) {
    ...
    controls.autoRotateSpeed = prefersReducedMotion ? 0.05 : 0.4;
    ...
    // === SHAPE DEFINITIONS ===
    const PARTICLE_COUNT = isMobileDevice ? 8000 : 25000;
    const shapes = [];
    ...
    const material = new THREE.ShaderMaterial({
        ...
        vertexShader: `
            ...
            void main() {
                ...
                // Gentle uniform scale and smooth rotation connected to scroll
                float scrollInfluence = uScroll * 0.2;
                pos *= (1.0 + scrollInfluence);
                
                float swirlRot = uScroll * 2.0;
                ...
                vAlpha *= (1.0 - min(0.4, uScroll * 0.3));
            }
        `,
        fragmentShader: `
            varying vec3 vColor;
    ...
    const morphDuration = prefersReducedMotion ? 0.8 : 1.2;
    let morphStartTime = 0;
    const clock = new THREE.Clock();

    const currentStartPositions = new Float32Array(PARTICLE_COUNT * 3);
    currentStartPositions.set(geometry.attributes.position.array);

    function startMorph(targetIdx) {
        if (targetIdx === currentShape && !isMorphing) return;
        if (targetIdx === targetShape && isMorphing) return;
    ...
    window.addEventListener('scroll', () => {
        if (window.scrollY > 80 && autoMorphInterval) {
            clearInterval(autoMorphInterval);
            autoMorphInterval = null;
        }
    }, { passive: true });
    ...
    function getDocumentElementTop(el) {
        if (!el) return 0;
        const rect = el.getBoundingClientRect();
        return rect.top + window.scrollY;
    }

    // SCROLL-DRIVEN SHAPE TRANSITIONS
    function checkScrollMorph() {
        const scrollY = window.scrollY;
        const maxScroll = Math.max(document.body.scrollHeight, document.documentElement.scrollHeight) - window.innerHeight;
        if (maxScroll <= 0) return;
        const progress = scrollY / maxScroll;

        // Section-specific anchors for see-more.html
        const pricingEl = document.getElementById('pricing');
        const builderEl = document.getElementById('custom-builder');
        const aboutEl = document.getElementById('why-choose-us') || document.getElementById('about');

        let targetIdx = 0;
        if (pricingEl && builderEl && aboutEl) {
            const pricingTop = getDocumentElementTop(pricingEl) - window.innerHeight * 0.45;
            const builderTop = getDocumentElementTop(builderEl) - window.innerHeight * 0.45;
            const aboutTop = getDocumentElementTop(aboutEl) - window.innerHeight * 0.45;

            if (scrollY >= aboutTop) {
                targetIdx = 3; // Helix
            } else if (scrollY >= builderTop) {
                targetIdx = 2; // Diamond
            ...
            // General multi-section threshold for terms/privacy/other pages
            if (progress >= 0.70) {
                targetIdx = 3;
        ...
        if (targetIdx !== currentShape && targetIdx !== targetShape) {
            startMorph(targetIdx);
        }
    }
    ...
    function animate() {
        ...
        material.uniforms.uTime.value = prefersReducedMotion ? elapsed * 0.2 : elapsed;
        ...
        // Check if scroll position transitions the particle formation
        checkScrollMorph();
        
        // Camera pushback based on scroll
        camera.position.z = 3.5 + smoothScrollProgress * 3;
        particles.rotation.z = smoothScrollProgress * Math.PI;
        
        // Anchor to the right on desktop, center on mobile
        if (window.innerWidth > 768) {
            const vFov = camera.fov * Math.PI / 180;
            const visibleHeight = 2 * Math.tan(vFov / 2) * camera.position.z;
            const visibleWidth = visibleHeight * camera.aspect;
            particles.position.x = visibleWidth * 0.18;
        } else {
        ...
        if (isMorphing) {
            ...
            for (let i = 0; i < len; i++) {
                posArray[i] = currentStartPositions[i] + (tgtPositions[i] - currentStartPositions[i]) * morphProgress;
            }
            geometry.attributes.position.needsUpdate = true;

            if (rawProgress >= 0.25 && lastUIUpdate !== targetShape) {
